...
class TestRecoveryPassword:
    ...
    @allure.description('Переходим на страницу восстановления пароля, заполняем поле Е-mail, кликаем на кнопку \
                               "Восстановить", кликаем на значок глаза, проверяем, что поле стало активным')
    @pytest.mark.parametrize('any_driver', ['driver_firefox', 'driver_chrome'])
    def test_focused_password(self, request, any_driver):
        driver = request.getfixturevalue(any_driver)
        login_page = LoginPage(driver)
        login_page.check_recovery_password()
